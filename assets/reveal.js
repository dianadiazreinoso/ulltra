...
function RevealSection() {
  ...
  React.useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(nightInnerRef.current, { scale: 1.08, transformOrigin: "50% 45%" });
      gsap.set(dayInnerRef.current, { scale: 1.0, transformOrigin: "50% 45%" });
      gsap.set(nightWrapRef.current, { autoAlpha: 1, clipPath: "inset(0% 100% 0% 0%)" });
      ...
      gsap.to(vignetteRef.current, {
        opacity: 0.85, duration: 7, ease: "sine.inOut", repeat: -1, yoyo: true
      });
      ...
      tl.fromTo(nightWrapRef.current,
        ...
        { clipPath: "inset(0% 0% 0% 0%)", ease: "power1.inOut", duration: WIPE }, 0);
      tl.fromTo(nightInnerRef.current, { scale: 1.08 }, { scale: 1.0, duration: WIPE }, 0);
      tl.fromTo(dayInnerRef.current, { scale: 1.0 }, { scale: 1.06, duration: WIPE }, 0);
      ...
      tl.to(nightInnerRef.current, { scale: 1.12, ease: "power1.out", duration: SD }, S);
      ...
      mm.add("(max-width: 760px)", () => {
        ...
        const apply = () => {
          ...
          gsap.set(night, { clipPath: `inset(0% ${(1 - wipe) * 100}% 0% 0%)` });
          gsap.set(nightInnerRef.current, { scale: 1.08 - 0.08 * wipe });
          gsap.set(dayInnerRef.current, { scale: 1 + 0.06 * wipe });
        };
    ...
    <section
      ...
      <div className="reveal-pin" ref={pinRef}>
        <div className="reveal-frame" ref={frameRef}>
          {/* Base layer — DAY (sepia), sits beneath. Landscape on desktop, a
              bust-centred vertical crop on mobile (≤768px) via <picture>. */}
          <div className="reveal-layer reveal-layer--cyber">
            <div className="reveal-img" ref={dayInnerRef}>
              <picture>
                <source media="(max-width: 768px)" srcSet={window.__asset("assets/reveal-dia-movil.webp", "revealDayMobile")} />
                <img src={window.__asset("assets/reveal-dia.webp", "revealDay")} alt="" draggable="false" />
              </picture>
            </div>
          </div>

          {/* Top layer — NIGHT, revealed by the clip-path wipe. Same desktop /
              mobile <picture> swap so both layers share the identical crop. */}
          <div className="reveal-layer reveal-layer--classic" ref={nightWrapRef}>
            <div className="reveal-img" ref={nightInnerRef}>
              <picture>
                <source media="(max-width: 768px)" srcSet={window.__asset("assets/reveal-noche-movil.webp", "revealNightMobile")} />
                <img src={window.__asset("assets/reveal-noche.webp", "revealNight")} alt="" draggable="false" />
              </picture>
            </div>
          </div>
          ...
          {/* Headline overlay — above both images, left-aligned in the empty space */}
          <h2 className="reveal-headline" aria-label="Future has ancient roots">
            <span className="reveal-h-line">
              <span className="reveal-h-archivo">Future has</span>
            </span>
            <span className="reveal-h-line">
              <span className="reveal-h-ancient">ancient</span>
              <span className="reveal-h-archivo"> roots</span>
            </span>
          </h2>
        </div>
      </div>
