...
/* =============================================================================
   ...
   -----------------------------------------------------------------------------
   A faint Renaissance ground, a centred editorial title, and a fanned deck of
   overlapping cards. The white centre card is the focus; coloured panels and
   painted image cards fan out on either side. All motion is GSAP + ScrollTrigger
   on transform/opacity only:
     · ENTRANCE  — cards rise from y+120, scale .9, slight rotation, power4.out,
                   staggered from the edges inward so the centre lands last.
     · HOVER     — hovered card lifts (-20px) & scales (1.04), shadow grows, comes
                   to front; neighbours ease away; image zooms; list staggers in.
     · TILT      — cursor-driven 3D tilt (max 8°) with smooth interpolation.
   Responsive via gsap.matchMedia: full fan ≥1101px, reduced overlap on tablet,
  ...
  React.useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const buildFan = factor => {
        ...
        // ── Interactions ──
        // HOVER: the hovered card comes to the FRONT of the deck — it straightens
        ...
        /* ── Subtle pointer parallax across the whole deck ──
           Each card's inner .caps-card drifts by an amount proportional to its
