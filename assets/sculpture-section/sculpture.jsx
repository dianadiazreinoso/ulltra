...
const { motion, useMotionValue, useTransform } = FM;

const HEART_FRAME_COUNT = 26;
const heartFrameSrc = (i) =>
...
function ArchetypeSection() {
  ...
  useEffect(() => {
    ...
    let targetFrame = 0, curFrame = 0, raf = 0;
    let cw = 0, ch = 0, shownFrame = -1;
    ...
    if (stage) {
      ...
      shrinkTl = gsap.timeline({ defaults: { ease: "none" },
        scrollTrigger: { trigger: sectionRef.current, start: "top top", end: "bottom bottom", scrub: 1, invalidateOnRefresh: true } });
    ...
    return () => {
      ...
      st.kill();
      enterST.kill();
      if (shrinkTl) { shrinkTl.scrollTrigger && shrinkTl.scrollTrigger.kill(); shrinkTl.kill(); }
    };
  }, []);

  useEffect(() => {
    const id = requestAnimationFrame(() => { if (resizeRef.current) resizeRef.current(); });
  ...
  return (
    <section className="sc-root" ref={sectionRef} data-header="dark" data-screen-label="Archetype">
      <div className="sc-stage">
        ...
        <div className="sc-caption" style={{ width: mobile ? "min(88vw, 380px)" : "min(80vw, 860px)", gap: "clamp(1px, 0.25vw, 4px)", marginTop: mobile ? "15px" : 0 }}>
          <motion.h2
            className="sc-cap-head"
            initial={{ opacity: 0, y: 26, filter: "blur(12px)" }}
            animate={entered ? { opacity: 1, y: 0, filter: "blur(0px)" } : { opacity: 0, y: 26, filter: "blur(12px)" }}
            transition={{ duration: 0.9, ease: EASE_CINE }}
            style={{ whiteSpace: "nowrap", fontSize: mobile ? "clamp(40px, 12vw, 62px)" : "100px", lineHeight: 0.95 }}>
            Old<br />wisdom
          </motion.h2>
          <motion.p className="sc-cap-sub" style={subStyle}>
            <em>new<br />instruments</em>
          </motion.p>
        </div>
      </div>
    </section>
